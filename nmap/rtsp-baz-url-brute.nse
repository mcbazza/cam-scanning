...
---
-- @usage
-- nmap --script rtsp-baz-url-brute -p 554,8554,10554 <ip>
--
-- @output
-- PORT    STATE SERVICE
-- 554/tcp open  rtsp
-- | rtsp-baz-url-brute:
-- |   discovered:
-- |     rtsp://camera.example.com:port/mpeg4
-- |   other responses:
-- |     401:
-- |_      rtsp://camera.example.com:port/live/mpeg4
-- @xmloutput
-- <table key="discovered">
--   <elem>rtsp://camera.example.com:port/mpeg4</elem>
-- </table>
-- <table key="other responses">
--   <table key="401">
--     <elem>rtsp://camera.example.com:port/live/mpeg4</elem>
--   </table>
-- </table>
--
-- @args rtsp-baz-url-brute.urlfile sets an alternate URL dictionary file
-- @args rtsp-baz-url-brute.threads sets the maximum number of parallel threads to run
...
--
-- Version 0.2
-- Created 23/10/2011 - v0.2 - Modified by bazza <@mcbazza> - allow scanning on 554, 8554, 10554. And display port in URL results.
--
...
-- Baz 25/09/2023 Let it run against more ports than the default 554/tcp.
-- For RTSP cams that aren't on 554/tcp, see: https://www.shodan.io/search?query=-port%3A554+has_screenshot%3Atrue+rtsp
-- portrule = shortport.port_or_service(554, "rtsp", "tcp", "open")
portrule = shortport.port_or_service({554, 8554, 10554}, {"rtsp", "rtsp-alt"}, "tcp", "open")
...
local function processURL(host, port, url_iter, result)
  ...
  for u in url_iter do
    -- Baz 25/09/2023 Add the port number to the URL
    -- local url = ("rtsp://%s%s"):format(name, u)
    local url = ("rtsp://%s:%s%s"):format(name, tostring(port.number), u)
...
action = function(host, port)
  ...
  local result = {}
  local condvar = nmap.condvar(result)
  local threadcount = stdnse.get_script_args('rtsp-baz-url-brute.threads') or 10
  local filename = stdnse.get_script_args('rtsp-baz-url-brute.urlfile') or
